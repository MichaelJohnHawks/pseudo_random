##  program name:
##  "pseudo_random_37_bit_simple.py"
##  language: Python 3
...
##  37 bit register (bits numbered
##  0 through 36) with tap points
##  at 0, 32, 33, 34, 35 and 36
##  with the register shifting one
##  bit to the left (toward the
##  higher bits) with each
##  iteration. The leftmost bit
##  (highest order bit) is shifted
##  out and discarded.
##
...
##  37 bit number loaded into the
##  register (in our simple case
##  the seed is thirty-six "0"
##  characters followed by one "1."
...
##  integers in the range 1 through
##  137,438,953,471 (which is
##  2**37-1). Zero never occurs.
##  Each integer occurs once and
##  only once in the sequence. After
##  137,438,953,471 cycles the
##  sequence repeats in exactly the
##  same order.
####################################
##  Interesting observation:
##  If you were to develope a
##  program that generated one
##  million pseudo-random integers
##  per second, it would take over
##  36 hours and 10 minutes before
##  this sequence would
##  repeat itself.
####################################
...
def fnc_next_random_binary_37_bit_string(str_seed):

    ##  This function accepts as an argument a 37 character string of zeros
    ##    and ones representing the 37 bit binary "seed."
    ##  Using the LFSR method of generating the next binary number in
    ##    the sequence, the parity of positions 0, 32, 33, 34, 35 and 36 is
    ##    determined. When parity is even then the new bit represented by
    ##    "str_x" will be "0" else the parity is odd and the new bit
    ##    will be "1."
    ##  Determining the parity is fairly easy and there are probably lots of
    ##    ways to do it. Here we start by comparing positions 0 and 32. If
    ##    they are not equal then parity is odd and "str_x" will be "1."
    ##    Next similarly compare "str_x" to position 33, position 34,
    ##    position 35, and finally to position 36.
    ##  Once parity of positions 0, 32, 33, 34, 35 and 36 is determined,
    ##    "shift" the bit represented by "str_x" into the right side of the
    ##    seed string and discard the leftmost position. This is now the
    ##    binary image of the next pseudo-random number in the sequence AND
    ##    becomes the new seed for the next time around.

    if str_seed[0] != str_seed[32]: ##  Find parity of positions 0 and 32
        str_x = "1"                 ##  If not equal, parity is odd or "1"
    else:                           ##    ELSE
        str_x = "0"                 ##  If equal, parity is even or "0"

    if str_seed[33] != str_x:       ##  Compare position 33 to previous result
        str_x = "1"                 ##  If not equal, parity is odd or "1"
    else:                           ##    ELSE
        str_x = "0"                 ##  If equal, parity is even or "0"

    if str_seed[34] != str_x:       ##  Compare position 34 to previous result
        str_x = "1"                 ##  If not equal, parity is odd or "1"
    else:                           ##    ELSE
        str_x = "0"                 ##  If equal, parity is even or "0"

    if str_seed[35] != str_x:       ##  Compare position 35 to previous result
        str_x = "1"                 ##  If not equal, parity is odd or "1"
    else:                           ##    ELSE
        str_x = "0"                 ##  If equal, parity is even or "0"

    if str_seed[36] != str_x:       ##  Compare position 36 to previous result
        str_x = "1"                 ##  If not equal, parity is odd or "1"
    else:                           ##    ELSE
        str_x = "0"                 ##  If equal, parity is even or "0"


    str_result = str_seed[1:37] + str_x  ##  "Shift" parity bit into the right
                                         ##    side of the string and discard
                                         ##    the leftmost bit.

    return str_result  ##  Return this result. This string is the binary image
                       ##    of the next pseudo-random integer in the sequence.

######################################################
######################################################

def fnc_display_farewell_to_user():  ##  Farewell message to user
...
def fnc_main_body():
        ...
        ##  This string represents the 37 bit
        ##    binary image of the initial seed:

    str_seed = "0000000000000000000000000000000000001"
    ...
    while str_input == "":  ##  Enter the loop. As long as the user keeps
        ...
        str_new = fnc_next_random_binary_37_bit_string(str_seed)
...
def fnc_display_greeting_to_user():  ##  Greeting message to user
    ...
    print('The initial seed is "0000000000000000000000000000000000001"')
    print()
    print("Using 37 bit binary integers the LFSR method")
    print("can generate all the integers from 1 through 137438953471")
    ...
    print("repeat itself.")
    print()
    print("THE SEED CAN NEVER BE ZERO OR ELSE NOTHING WILL HAPPEN!!!")
    print()
