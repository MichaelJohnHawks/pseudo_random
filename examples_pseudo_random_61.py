##  program name:
##  "examples_pseudo_random_61_bit.py"
##  language: Python 3
##  by Michael John Hawks
##  2020-05-09
###################################
##  Example routines that use
##  65 bit binary pseudo-random
##  integers
###################################
...
##  61 bit register (bits numbered
##  0 through 60) with tap points
##  at 0, 1, 15 and 16 with the
...
##  61 bit binary number loaded
##  into the register. The seed
##  must be non-zero.
##
...
##  (which is (2**61)-1). Zero
##  never occurs. Each integer
...
def fnc_next_random_binary_61_bit_string(str_seed):
    ...
    ##    the sequence, the parity of positions 0, 1, 15 and 16 is
    ...
    ##    Next similarly compare "str_x" to position 15 and finally to
    ##    position 16.
    ##  Once parity of positions, 0, 1, 15 and 16 is determined, "shift" the
    ...
    if str_seed[15] != str_x:       ##  Compare position 15 to previous result
        str_x = "1"                 ##  If not equal, parity is odd or "1"
    else:                           ##    ELSE
        str_x = "0"                 ##  If equal, parity is even or "0"

    if str_seed[16] != str_x:       ##  Compare position 16 to previous result
        str_x = "1"                 ##  If not equal, parity is odd or "1"
    else:                           ##    ELSE
        str_x = "0"                 ##  If equal, parity is even or "0"


    str_result = str_seed[1:61] + str_x  ##  "Shift" parity bit into the right
...
def fnc_convert_integer_to_binary_string_image(int_n):
    ...
    ##
    ##  The string will always be 61 characters long.
    ##
    ##  If the integer is larger than what 61 bits can represent,
    ##  this function stops at 61 characters and calls it good.

    str_binary_image = ""  ##  Start with an empty sring to build upon

    int_i = 0  ##  Initialize loop

    while int_i < 61:  ##  61 times through loop required
    ...
    return str_binary_image  ##  Return the string of 61 characters
...
def fnc_pseudo_random_1_thru_n(int_n,str_seed):
    ...
    ##  1 through n (as long as "n" doesn't exceed (2**n)-1 or
    ##  2,305,843,009,213,693,951). Each returned integer has an equal
    ...
    ##  Initialize the loop by starting with the
    ##  largest allowed integer which is (2**n)-1.
    int_trial = 2305843009213693951
    ...
    ##  is  but there's a really good chance that 2305843009213693951 is
    ##  NOT an exact multiple of "n". So we'll subtract
    ##  2305843009213693951 modulo n from 2305843009213693951
...
def fnc_deal_poker_hands(str_seed):
    ...
    ##  of cards in a freshly shuffled deck.
    ##
    ##  A 61 bit binary register can generate only (2**n)-1
    ##  or 2305843009213693951 states. A deck of 52 cards can
    ##  be sequenced
...
def fnc_pseudo_random_integers(str_seed):
    ...
    while str_input == "":  ##  Enter the loop. As long as the user keeps
                            ##    hitting <ENTER>, "str_input" will be an
                            ##    empty string and the loop will continue.
        ...
        ##    be an empty string and the loop will continue. Any other
        ##    response will cause the loop to terminate.

    return str_seed  ##  Updated seed string is returned
                     ##  and function terminates

######################################################
######################################################

def fnc_select_seed(str_seed):
    ...
    print("*******************************************************")
    print("The SEED is an integer between 0 and")
    print("2,305,843,009,213,693,951.")
    print()
    ...
    print()
    print("To select a SEED based on an elapsed time algorithm")
    ...
    while int_seed <= 0:  ##  Keep looping as long as seed value is invalid.
        ...
        if str_input == "":  ##  Empty string signals to use an elapsed
                             ##  time algorithm as follows:

            int_x = int(time.clock()*1000000)

            ##  "time.clock" returns the number of seconds since the
            ##  clock became active (when this function was called)
            ##  represented as a float value. This value is supposedly accurate
            ##  down to a millionth of a second.
            ##
            ##  For this algorithm I decided to get an integer by multipling
            ##  the "time.clock" value by 1,000,000 (to get the elapsed
            ##  milliseconds) then converting that to a string representing
            ##  the integer as binary.
            ##
            ##  So now what? Finally I decided to take the 22 least significant
            ##  bytes at the end of the string (representing a little over 4
            ##  seconds of elapsed time), append them together three times,
            ##  take the 60 least significant bytes of THAT and finally append
            ##  "1" to the end. That gives me a string 61 characters long
            ##  that hopefully has a good mix of 0's and 1's and will never
            ##  represent zero.
            ##
            ##  There's probably better ways to do this. Go ahead and take
            ##  a shot at doing something more coherent.

            str_x = (fnc_convert_integer_to_binary_string_image(int_x))[-22:]
            str_seed = (str_x+str_x+str_x)[-60:] +"1"
            ...
            try:
                int_seed = 1+((int(str_input)-1) % 2305843009213693951)
                ...
                ##  integer by subtracting 1, finding modulo
                ##  2305843009213693951 (which yields a value of 0 through
                ##  2305843009213693951), then add 1. This gives us a final
                ##  value that is non-zero and within the required range.
...
def fnc_user_menu(str_seed):
    ...
    if int_option == 1:  ##  Choice #1: display pseudo-random integers
        ...
        str_seed = fnc_select_seed(str_seed)
...
def fnc_user_interaction_loop():
    ...
    ##  Set up an initial "seed"
    str_seed="1010011100101110111001010011100101110111001010011100101110111"
...
def fnc_display_greeting_to_user():
    ...
    print("using a 61 character string of zeros and ones")
    print("to represent the integer as binary.")
    print()
    print("Using 61 bit binary integers the LFSR method")
    print("can generate all the integers from 1 through")
    print("2,305,843,009,213,693,951 in a pseudo-random")
    print("fashion (i.e. -- very difficult to predict the")
    print("next integer in the sequence).")
    print("Each integer appears once and only once throughout")
    print("the sequence at which point the sequence will")
    print("repeat itself.")
    print()
    print("THE SEED CANNOT BE ZERO OR ELSE NOTHING WILL HAPPEN!!!")
